...
bases:
- github.com/mortent/kpt-provisioning/bases/zookeeper?ref=bases/zookeeper/v0.1.0
...
p "# pull in an optional add-on with a networkpolicy for Zookeeper"
...
p "# commit the changes to my fork and tag the new version"
pe "git add bases && git commit -m \"Add networkpolicy\""
pe "git push origin master"
pe "git tag bases/zookeeper/v0.3.0"
pe "git push origin bases/zookeeper/v0.3.0"
wait

p "# update the dev configuration to use the new version of the fork"
pe "sed -i '' 's/v0.2.0/v0.3.0/g' envs/dev/zookeeper/kustomization.yaml"
